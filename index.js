/**
 * Created by linguochi on 2017/4/18.
 */
...
module.exports = function (content, file, conf) {
    ...
    var ignoreFiles = [];
    if (conf.ignoreFiles) {
        if (typeof conf.ignoreFiles === 'string' || fis.util.is(conf.ignoreFiles, 'RegExp')) {
            ignoreFiles = [conf.ignoreFiles];
        } else if (fis.util.is(conf.ignoreFiles, 'Array')) {
            ignoreFiles = conf.ignoreFiles;
        }
    ...
    if (ignoreFiles) {
        for (var i = 0, len = ignoreFiles.length; i < len; i++) {
            if (fis.util.filter(file.subpath, ignoreFiles[i])) {
                return;
            }
        }
    }
